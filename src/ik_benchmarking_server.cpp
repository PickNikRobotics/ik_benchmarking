...
IKBenchmarkingServer::IKBenchmarkingServer(const rclcpp::NodeOptions& options)
    ...
    this->action_server_ = rclcpp_action::create_server<IKBenchmark>(
        this, "ik_benchmark", std::bind(&IKBenchmarkingServer::handle_goal, this, _1, _2),
        std::bind(&IKBenchmarkingServer::handle_cancel, this, _1),
        std::bind(&IKBenchmarkingServer::handle_accepted, this, _1));
}

rclcpp_action::GoalResponse IKBenchmarkingServer::handle_goal(
    const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const IKBenchmark::Goal> goal) {
    RCLCPP_INFO(this->get_logger(), "Received IKBenchmark goal request with solver %s",
                goal->solver_name.c_str());
    (void)uuid;
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse IKBenchmarkingServer::handle_cancel(
...
void IKBenchmarkingServer::handle_accepted(
    const std::shared_ptr<GoalHandleIKBenchmark> goal_handle) {
    using namespace std::placeholders;

    bool execute_once{true};  // Shutdown server node after handling one action goal to enable
                              // sequencing while collecting IK solution data
    std::thread{std::bind(&IKBenchmarkingServer::execute, this, _1, _2), goal_handle, execute_once}
        .detach();
}
...
void IKBenchmarkingServer::execute(const std::shared_ptr<GoalHandleIKBenchmark> goal_handle,
    ...
    auto progress =
        std::make_shared<IKBenchmark::Feedback>();  // Todo: Mohamed, handle the progress feedback
    ...
    IKBenchmarking ik_benchmarker("ik_benchmarker", goal->solver_name.c_str(),
                                  goal->csv_filename.c_str(), this->get_node_options());
    ik_benchmarker.run();
